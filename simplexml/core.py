...
def element_from_dict(document, elRoot, data):
    ...
    for k, v in data.items():
        ...
        if isinstance(v, dict):
            ...
            if '_value' in v:
                value = v.get('_value')
                textNode = document.createCDATASection(value) if isinstance(value, str) and re.search("[\<\>\&]", value) else document.createTextNode(str(v) if type(v) else unicode(v))
            ...
            elem = document.createElement(k)
            elem.appendChild(document.createTextNode(str(v) if type(v) else unicode(v)))
            elRoot.appendChild(elem)
